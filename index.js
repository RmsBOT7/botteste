...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adeus @${num.split('@')[0]} ja foi tarde 😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["556599071373@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTdPIVwxTKTEAfLTZKmSY-sub3HW4RFDfX_gA&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* \n*WPP:* wa.me/+556599071373\n*INSTA:* @guilhermetga1993 '})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
